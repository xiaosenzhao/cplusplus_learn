...
constexpr std::string_view get_enum_value_name() {
  std::string_view name{PRETTY_FUNCTION_NAME,
                        sizeof(PRETTY_FUNCTION_NAME) - OFFSET};
...
constexpr auto test_integer_sequence_v =
    make_integer_list_wrapper(std::make_integer_sequence<int, 256>());

template <typename E, int... Is>
constexpr size_t get_enum_size(std::integer_sequence<int, Is...>) {
  constexpr std::array<bool, sizeof...(Is)> valid{
      is_valid<E, static_cast<E>(Is)>()...};
  constexpr std::size_t count =
      [](decltype((valid)) valid_) constexpr noexcept -> std::size_t {
...
constexpr auto get_all_valid_names(std::integer_sequence<int, Is...>) {
  constexpr std::array<std::string_view, sizeof...(Is)> names{
      get_enum_value_name<E, static_cast<E>(Is)>()...};
...
constexpr auto static_throw(int n) -> void {
  n <= 0 ? throw std::runtime_error("should not reach here. Invalid value.")
         : 0;
}
...
int main() {
  ...
  std::cout << "RED: " << static_cast<int>(string2enum<Color>("RED"))
            << std::endl;
  std::cout << "BLUE: " << static_cast<int>(string2enum<Color>("BLUE"))
            << std::endl;
  std::cout << "GREEN: " << static_cast<int>(string2enum<Color>("GREEN"))
            << std::endl;
  // throw exception, LOL!
  // std::cout << "GREEN: " << static_cast<int>(string2enum<Color>("GRAY")) <<
  // std::endl;
